...
class Model(nn.Module):
    def __init__(self, num_class=200, num_point=46, num_person=1, graph=None, graph_args=dict(), 
        ...
        # --- LOGIC TỰ ĐỘNG CHỌN SỐ KÊNH ---
        self.select_stream = select_stream
        if self.select_stream in ['joint', 'velocity', 'bone']:
            self.actual_in_channels = 3 # Nếu chọn 1 luồng thì input luôn là 3
        else:
            self.actual_in_channels = 9 # Mặc định là 'all' (9 kênh)

        self.num_point = num_point
        # BatchNorm khởi tạo theo số kênh thực tế sau khi cắt
        self.data_bn = nn.BatchNorm1d(num_person * self.actual_in_channels * num_point)

        # Baseline CTR-GCN Backbone (Shared Stem)
        # Input layer l1 nhận actual_in_channels (3 hoặc 9)
        self.l1 = TCN_GCN_Unit(self.actual_in_channels, base_channel, A, residual=False)
    ...
    def forward(self, x):
        # x shape gốc từ loader: (N, 9, T, V) hoặc (N, 9, T, V, M)
        
        # Fix Input Shape nếu thiếu dimension M
        if len(x.shape) == 4:
            x = x.unsqueeze(-1)
        
        # --- CẮT LUỒNG DỮ LIỆU (STREAM SLICING) ---
        if self.select_stream == 'joint':
            x = x[:, :3, :, :, :]       # Lấy 3 kênh đầu (Joint)
        elif self.select_stream == 'velocity':
            x = x[:, 3:6, :, :, :]      # Lấy 3 kênh giữa (Velocity)
        elif self.select_stream == 'bone':
            x = x[:, 6:9, :, :, :]      # Lấy 3 kênh cuối (Bone)
        # Nếu 'all' thì giữ nguyên 9 kênh
